...
const BigCalendar = (props: BigCalendarProps) => {
  ...
  const [currentDate, setCurrentDate] = useState(props.date ? moment(props.date) : moment());
  ...
  const generateMonthDays = () => {
    ...
    for (let i = startDay - 1; i >= 0; i--) {
      days.push({
        date: moment([prevMonth.year(), prevMonth.month(), daysInPrevMonth - i]),
  ...
  const getEventsForDate = (date: moment.Moment) => {
    return visibleEvents.filter(event => {
      ...
      return dayDate.isSame(eventStart) || 
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="grid grid-cols-7 text-center font-medium py-2">
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
            <div key={day} className="text-gray-200">{day}</div>
          ))}
        </div>
              ...
              {week.map((day, dayIndex) => {
                ...
                return (
                  <div 
                    ...
                    className={`
                      p-1 border hover:bg-gray-700 cursor-pointer overflow-hidden flex flex-col
                      ${day.isCurrentMonth ? 'border-gray-600 text-white' : 'border-gray-800 text-gray-500'}
                  ...
                  >
                    ...
                    <div className="flex-grow overflow-y-auto space-y-1">
                      {dayEvents.map((event) => {
                        ...
                        const eventStyle = props.eventPropGetter 
                          ? props.eventPropGetter(event).style 
    ...
    >
      ...
      <div className="flex justify-between items-center mb-3 p-2 bg-gray-800">
        ...
        <div className="flex space-x-2">
          <button
            onClick={() => handleViewChange('month')}
            className={`px-3 py-1 rounded ${currentView === 'month' 
              ? 'bg-blue-600' 
          ...
          <button
            ...
            className={`px-3 py-1 rounded ${currentView === 'week' 
              ? 'bg-blue-600' 
              : 'bg-gray-700 hover:bg-gray-600'} text-white`}
          >
            Week
          ...
          <button
            ...
            className={`px-3 py-1 rounded ${currentView === 'day' 
              ? 'bg-blue-600' 
              : 'bg-gray-700 hover:bg-gray-600'} text-white`}
          >
            Day
